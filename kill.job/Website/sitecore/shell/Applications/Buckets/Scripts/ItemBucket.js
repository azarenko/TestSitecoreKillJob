...
function buildGridViewField(name, value) {
    return '<p class="buckets-field" title="{0}: {1}"><strong>{0}:</strong>&nbsp{1}</p>'.scFormat(name, value);
}
...
function renderGridView(data) {
    ...
    $j.each(data.items,
        function () {
            ...
            var template = '<div onclick="{0}" class="post_float rounded" title="' + this.Name + '" style="' + Meta(this) + '">' +
                '<a class="ceebox imgcontainer" title="' + this.Name + '" href="#" onclick="{0}"><img onerror="this.onerror=null;this.src=\'../Buckets/images/default.jpg\';" src="' + this.ImagePath + '?w={1}&h={2}&db=master" class="attachment-post-thumbnail wp-post-image {3}" alt="' + this.Name + '" title="' + this.Name + ' - ' + this.Path + '" /></a>' +
                '<h2><a class="ceebox" title="' + this.Name + '" href="" onclick="{0}">' + this.Name + '</a></h2>' +
                '<div class="post_tags">' +
                    buildGridViewField(templateStub, this.TemplateName) +
                    buildGridViewField(locationStub, this.Bucket) +
                    '<p>' + (this.Content ? this.Content : "") + '</p>' +
                    buildGridViewField(versionStub, this.Version) +
                    buildGridViewField(createdStub, this.CreatedDate.substring(0, 10)) +
                    buildGridViewField(byStub, this.CreatedBy) +
                    buildGridViewField(languageStub, this.Language) +
                    '</div>' +
            '</div>';
            switch (window.currentBucketsViewType) {
                case window.rteViewType:
                ...
                $j.each(a.items,
                    function (serverItem) {
                        if (a.items != 0) {
                            ...
                            var contentFiller = (this.Content ? this.Content : "");
